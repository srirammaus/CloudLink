...
class Weather_worker {
    ...
    public function work ($coords) {
        if(is_array($coords)){
            ...
            //merge array
            $merged_coords = array_merge($nearby_location_coords,$current_coord);  //$nearby_top_cities_coords  - run this seprately and put in general
            
            // var_dump($merged_coords);
            $responses = $weather->fetchNearbyData($merged_coords);
            if($responses){
