...
function logg($file,$message=null,$exception_=null,$type = "err"){ // this type err is used for future, if we getting postive logs we can this as 
    ...
    $root_dir = !empty($_SERVER['DOCUMENT_ROOT']) ? $_SERVER['DOCUMENT_ROOT']: __DIR__."/..";
    if($exception_) { 
    ...
    switch ($file) {
        ...
        case 'redis_err': 
            ...
            break;
        case 'kafka_err': 
            $file = fopen("$root_dir"."/error_log/kafka_err.log","a+");
            fwrite($file,$message."\n\n\n");
            break;
        case 'event_log': 
            $file = fopen("$root_dir"."/error_log/event.log","a+");
            fwrite($file,$message."\n\n\n");
            break;
        case "backend_log":
