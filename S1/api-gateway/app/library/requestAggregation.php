...
// Feature	          Async Fire & Continue	    Guzzle Pool
// Start All Requests	    ✅	                ✅
// Control When to Wait	✅ You choose	❌ Waits at pool end

// Concurrency Limit	 ❌ You must manage manually	✅ Built-in
// Partial Result Handling	✅ Easier	✅ via fulfilled callback
// Simplicity	        ✅ Straightforward	Slightly complex for small cases
...
/**
 * Request aggregation is a design pattern where an intermediate system
 *  (like an API gateway or aggregator service) receives multiple individual client requests, fans them out to various 
 * backend services, collects their responses, and compiles them into a single, unified response sent back to the client
 * 
 */
class requestAggregation {
    ...
    public function setAgURLs($route_name) { //eg:dashboard

        $endpoint_data =  new EndpointData;
        //for ulitple rows use get+ toArray
        $data = $endpoint_data->where('name',$route_name)->first();
        
        if($data) {
            $this->route_name =  $data->name;

            $ports = $data->port;
            $this->ports = explode("-",$ports);
            ...
            $auths = $data->auth;
            $this->auths = explode("-",$auths);

            $baseURLs = $data->baseURL;
            $this->baseURLs = explode("-",$baseURLs);
            ...
            for($i=0;$i<count($this->paths);$i++) {
                $edpoint = new edpoint;
                $edpoint->path = $this->paths[$i];
                $edpoint->port = $this->ports[$i];
                $edpoint->baseURL = $this->baseURLs[$i];
                $edpoint->auth = $this->auths[$i];
                $edpoint->method = $this->methods[$i];
                array_push($sub_req_endpoints,$edpoint);


            }
            return $sub_req_endpoints;
     ...
     */
    public function init_req ($request,$promisables,$edpoint) {
        try {
            ...
            if($this->auth == '1') {
                if(authenticate::isAuthenticated($this->headers)) { //let the user in if only authenticated    
                    ...
                    array_push($promisables,$endpoint->requestAsync(
                        $this->method,
                        $this->path,
                        $this->dr_content));
    ...
    public function set_data($endpoint_data) {

        if($endpoint_data){
            $this->url = $endpoint_data->baseURL;
            $this->route_name = $endpoint_data->name;
            $this->path = $endpoint_data->path;
            array_push($this->sub_req_paths,$this->path);  //adding the actual path to sub req path stack
